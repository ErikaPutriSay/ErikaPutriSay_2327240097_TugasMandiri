...
import 'package:informasigedungserbaguna/models/post.dart'; // Sesuaikan jika nama modelnya berubah menjadi post.dart
import 'package:informasigedungserbaguna/screens/add_post_screen.dart';
import 'package:informasigedungserbaguna/screens/map_detail_screen.dart';
...
class _DetailScreenState extends State<DetailScreen> {
  ...
  void initState() {
    ...
    _reviewStream = widget.post.id != null
        ? PostService.getReviewsForGedungSerbaguna(widget.post.id!)
        : Stream.value([]);
  }
  ...
  Future<void> _toggleFavorite() async {
    ...
    if (_isFavorite) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Gedung serbaguna dihapus dari favorit.')),
      );
    } else {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Gedung serbaguna ditambahkan ke favorit.'),
        ),
      );
    }
  }
  ...
  Future<void> _deletePost(BuildContext context) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text('Hapus Gedung Serbaguna'),
        content: const Text(
  ...
  Widget build(BuildContext context) {
    ...
    // Cek apakah user yang membuka ini adalah admin pemilik postingan gedung serbaguna
    final isAdmin =
        currentUserId != null && widget.post.userId == currentUserId;

    return Scaffold(
      appBar: AppBar(
        title: Text(widget.post.name ?? 'Detail Gedung Serbaguna'),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    widget.post.description ??
                        'Belum ada deskripsi untuk gedung serbaguna ini.',
                    style: const TextStyle(
                      fontSize: 15,
                      height: 1.4,
                            ...
                            final result = await Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (_) => AddPostScreen(
                                  isAdmin: false,
                                  gedungSerbaguna:
                                      widget.post, // Kirim data tempat saat ini
                                ),
                              ),
                            );
