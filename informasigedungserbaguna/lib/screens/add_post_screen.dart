...
class AddPostScreen extends StatefulWidget {
  final bool
  isAdmin; // True: Admin tambah tempat, False: Pengguna tambah ulasan
  final Post?
  gedungSerbaguna; // diperlukan jika status isAdmin = false (Pengguna)

  const AddPostScreen({super.key, required this.isAdmin, this.gedungSerbaguna});
...
class _AddPostScreenState extends State<AddPostScreen> {
  ...
  Future<void> _getLocation() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Gagal mengambil lokasi gedung serbaguna."),
        ),
      );
    } finally {
      if (mounted) {
  ...
  Widget _buildImagePreview() {
    if (_base64Image == null) {
      return Container(
        ...
        child: Text(
          widget.isAdmin
              ? 'Belum ada foto gedung serbaguna dipilih'
  ...
  Future<void> _submitPost() async {
    ...
    if (widget.isAdmin) {
      final isEditingNow = _isEditing && widget.gedungSerbaguna != null;
      // ---------------- LOGIKA SUBMIT ADMIN ----------------
      if (_nameController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Masukkan nama gedung serbaguna.')),
        );
        return;
      }
      if (_base64Image == null) {
      ...
      try {
        ...
        if (_latitude == null || _longitude == null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Harap ambil atau masukkan lokasi gedung serbaguna terlebih dahulu.',
              ),
            ),
          );
          return;
        ...
        final postToSave = Post(
          id: isEditingNow ? widget.gedungSerbaguna!.id : null,
          image: _base64Image,
        ...
        if (isEditingNow) {
          await PostService.updatePost(postToSave);
        } else {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Data Gedung Serbaguna berhasil disimpan"),
          ),
        );
      ...
      if (widget.gedungSerbaguna == null ||
          widget.gedungSerbaguna!.id == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Data Gedung Serbaguna tidak valid.')),
        );
        return;
      }
      ...
      try {
        ...
        await PostService.addReviewToGedungSerbaguna(
          gedungSerbagunaId: widget.gedungSerbaguna!.id!,
          reviewData: {
            'userId': adminId,
  ...
  void initState() {
    super.initState();
    // If a gedungSerbaguna is provided and we're in admin mode, prefill fields for editing
    if (widget.isAdmin && widget.gedungSerbaguna != null) {
      _isEditing = true;
      final p = widget.gedungSerbaguna!;
  ...
  // 7. Fungsi AI: Generate deskripsi menarik otomatis berdasarkan foto gedung serbaguna
  Future<void> _generateDescriptionWithAI() async {
    if (_base64Image == null) return;
    setState(() => _isGenerating = true);
    try {
      const apikey = 'AQ.Ab8RN6I6jELd2g_HQiM6QD4csXZbLoaPfllKy_9B1uSXMXiYUA';
      const url =
  ...
  Future<void> sendNotificationToTopic(String body, String senderName) async {
    final url = Uri.parse(
      'https://erika-putri-say-2327240097-tugas-ma.vercel.app/send-to-topic',
    );
    try {
      await http.post(
        url,
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          "topic": "gedung-serbaguna-palembang",
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.isAdmin
              ? (_isEditing
                    ? 'Edit Gedung Serbaguna'
                    : 'Tambah Gedung Serbaguna Baru')
              : 'Tambah Ulasan Baru',
        ),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            if (widget.isAdmin) ...[
              // Input Nama Gedung Serbaguna
              TextField(
                controller: _nameController,
                decoration: const InputDecoration(
                  labelText: 'Nama Gedung Serbaguna',
              ...
              TextField(
                ...
                maxLines: 4,
                decoration: const InputDecoration(
                  labelText: 'Deskripsi Gedung Serbaguna',
                  hintText:
            ...
            if (!widget.isAdmin) ...[
              Text(
                "Berikan Ulasan untuk:\n${widget.gedungSerbaguna?.name ?? 'Gedung Serbaguna'}",
                textAlign: TextAlign.center,
                style: const TextStyle(
                  fontSize: 16,
                  fontWeight: FontWeight.bold,
